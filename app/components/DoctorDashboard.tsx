...
function StatCard({ label, value, icon: Icon, color, bgLight, onClick }: { label: string; value: number | string; icon: any; color: string; bgLight: string; onClick?: () => void }) {
  return (
    <div 
      ...
      onClick={onClick}
    >
      <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center mb-2 sm:mb-3 ${bgLight}`}>
        <Icon size={16} className={color} />
      </div>
      <p className="font-mono-custom text-xl sm:text-3xl font-bold text-surface-900 mb-0.5">{value}</p>
      <p className="font-body text-[10px] sm:text-xs text-surface-500 uppercase tracking-wider font-medium leading-tight">{label}</p>
    </div>
  );
}
      ...
      <div className="flex min-h-screen max-w-full overflow-x-hidden">
        ...
        <header className="lg:hidden fixed top-0 left-0 right-0 z-20 bg-white border-b border-surface-200 px-4 py-3 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex flex-col items-start gap-0.5 min-w-0">
              <span className="font-display font-bold text-surface-900 leading-none text-sm">MediQueue</span>
              <InstallPWA />
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-2">
            <div className={`flex items-center gap-1.5 text-[10px] font-body font-semibold px-2.5 py-1 rounded-full border ${regWindow.isOpen ? "bg-brand-50 text-brand-700 border-brand-200" : "bg-surface-100 text-surface-500 border-surface-200"}`}>
              <div className={`w-1.5 h-1.5 rounded-full ${regWindow.isOpen ? "bg-brand-500 status-live" : "bg-surface-400"}`} />
              {regWindow.isOpen ? "Open" : "Closed"}
            </div>
            <button onClick={handleLogout} className="text-surface-500 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 transition-colors">
              <LogOut size={16} />
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 lg:ml-64 pt-16 lg:pt-0">
          <div className="p-4 sm:p-5 lg:p-8 max-w-5xl mx-auto">
            ...
            {/* Page header */}
            <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 mt-3 lg:mt-8 gap-3">
              <div className="min-w-0">
                {editName ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      autoFocus
                    />
                    <button onClick={() => setEditName(false)} className="p-1.5 hover:bg-surface-200 rounded-lg shrink-0">
                      <X size={18} className="text-surface-500" />
                    </button>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setEditName(true)}>
                    <h1 className="font-display text-2xl sm:text-3xl font-extrabold text-surface-900 tracking-tight truncate">{doctorName}</h1>
                ...
                <p className="font-body text-xs sm:text-sm text-surface-500 mt-1 font-medium">
              ...
              <button
                ...
                disabled={loading}
                className={`flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-body text-sm font-semibold transition-all shadow-md disabled:opacity-60 disabled:cursor-not-allowed shrink-0 ${
                  regWindow.isOpen
            ...
            {/* Tabs (mobile) */}
            <div className="flex lg:hidden gap-1 bg-surface-200/50 rounded-xl p-1 mb-5">
            ...
            <div className={tab !== "queue" ? "hidden lg:block" : ""}>
              {/* Currently Seeing */}
              {inProgress && (
                <div className="bg-linear-to-br from-brand-600 to-accent-600 rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-8 mb-6 text-white shadow-xl shadow-brand-500/20 relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between relative z-10 gap-4 sm:gap-6">
                    <div className="min-w-0">
                      <p className="font-body text-brand-100 text-xs uppercase tracking-widest font-semibold mb-1.5">Currently Seeing</p>
                      <p className="font-display text-4xl sm:text-5xl font-extrabold mb-1.5 tracking-tight">Token #{inProgress.token_number}</p>
                      <p className="font-body text-white font-medium text-base sm:text-lg truncate">{inProgress.name} · {inProgress.age}y · {inProgress.gender}</p>
                      <p className="font-body text-brand-100 text-sm mt-0.5 truncate">{inProgress.reason}</p>
                    </div>
                    <div className="flex sm:flex-col items-center gap-3 bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/20 shrink-0">
                      ...
                      <button
                        ...
                        className="bg-white text-brand-700 hover:bg-brand-50 border border-transparent rounded-xl px-4 sm:px-5 py-2 sm:py-2.5 font-body font-bold text-sm transition-all shadow-sm flex-1 sm:flex-none whitespace-nowrap"
                      >
                        Mark as Done
                      </button>
                    </div>
              ...
              {/* Call Next */}
              <div className="flex flex-col xs:flex-row xs:items-center justify-between mb-5 gap-3">
                <h2 className="font-display text-lg sm:text-xl font-bold text-surface-900">
                  Patient Queue <span className="font-body text-xs sm:text-sm text-surface-500 font-medium ml-1 bg-surface-100 px-2 py-1 rounded-full">{waiting.length} waiting</span>
                </h2>
                <div className="flex flex-col items-stretch xs:items-end gap-1 shrink-0 w-full xs:w-auto">
                  <button
                    onClick={callNext}
                    disabled={waiting.length === 0 || !!inProgress || loading}
                    className="flex items-center justify-center gap-2 bg-surface-900 hover:bg-surface-800 disabled:bg-surface-200 disabled:text-surface-400 disabled:cursor-not-allowed text-white text-sm font-body font-semibold px-4 sm:px-5 py-2.5 rounded-xl transition-all shadow-md disabled:shadow-none whitespace-nowrap w-full xs:w-auto"
                  >
                    <span>Call Next Patient</span>
                    <ChevronRight size={16} />
                  </button>
                  {inProgress && (
                    <p className="text-xs font-body text-amber-600 font-medium text-center xs:text-right">
              ...
              <div className="bg-white rounded-2xl border border-surface-200 shadow-sm overflow-hidden">
                {patients.length === 0 ? (
                  <div className="text-center py-16 sm:py-20">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-surface-100 rounded-full flex items-center justify-center mx-auto mb-4">
                      <Users size={24} className="text-surface-400" />
                    </div>
                    <p className="font-display text-base sm:text-lg font-bold text-surface-900 mb-1">Queue is empty</p>
                  ...
                  <div className="divide-y divide-surface-100">
                    {patients.map((p) => (
                      <div
                        key={p.id}
                        className={`p-3 sm:p-4 transition-all hover:bg-surface-50 ${
                          p.status === "done" || p.status === "skipped" ? "bg-surface-50/50 opacity-60" : ""
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center font-mono-custom font-bold text-sm sm:text-base shrink-0 ${
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                              <p className="font-body font-bold text-surface-900 text-sm sm:text-base truncate">{p.name}</p>
                              <span className={`text-[10px] px-2 py-0.5 rounded-full border font-body font-semibold uppercase tracking-wider shrink-0 ${statusBadge(p.status)}`}>
                                {p.status === "in-progress" ? "In Progress" : p.status}
                              </span>
                            </div>
                            <p className="font-body text-xs text-surface-500 font-medium truncate">
                              {p.age}y · {p.gender} · {p.phone} <span className="text-surface-300 mx-1">|</span> {p.reason}
                            </p>
                          </div>

                          <div className="text-right flex flex-col items-end gap-1.5 shrink-0">
                            <p className="font-mono-custom text-[10px] sm:text-xs font-medium text-surface-400">
                              {new Date(p.registered_at).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
                            </p>
                            {p.status === "waiting" && (
                              <button
                                onClick={() => skipPatient(p.id)}
                                className="flex items-center gap-1 text-[10px] sm:text-xs font-semibold text-surface-400 hover:text-red-500 hover:bg-red-50 px-2 py-1 rounded-lg transition-colors"
                              >
                                <SkipForward size={11} />
                                Skip
                              </button>
              ...
              <div className="animate-slide-up">
                <div className="bg-white rounded-2xl border border-surface-200 shadow-sm p-5 sm:p-6 lg:p-8 mb-6">
                  <div className="flex items-center gap-4 mb-6 sm:mb-8 pb-5 sm:pb-6 border-b border-surface-100">
                    ...
                    <div>
                      <h2 className="font-display text-xl sm:text-2xl font-bold text-surface-900">Session Settings</h2>
                      <p className="font-body text-xs sm:text-sm text-surface-500 mt-0.5">Configure today's OPD registration window and rules</p>
                    </div>
                  </div>
                ...
                <button
                  onClick={() => toggleRegistration(!regWindow.isOpen)}
                  disabled={loading}
                  className={`w-full rounded-2xl py-4 font-body font-bold text-base sm:text-lg flex items-center justify-center gap-3 transition-all shadow-lg disabled:opacity-60 disabled:cursor-not-allowed ${
                    regWindow.isOpen
                      ? "bg-red-500 hover:bg-red-600 text-white shadow-red-500/20"
                      : "bg-brand-600 hover:bg-brand-700 text-white shadow-brand-500/20"
                  }`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-surface-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-surface-200 flex flex-col max-h-[85vh] animate-slide-up">
            <div className="p-5 sm:p-6 border-b border-surface-100 flex items-center justify-between">
              <div>
                <h2 className="font-display text-lg sm:text-xl font-bold text-surface-900">Completed Patients</h2>
